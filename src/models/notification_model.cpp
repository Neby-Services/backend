...
NotificationModel::NotificationModel(std::string id, std::string type, std::string created_at, std::string updated_at, std::optional<NotificationServiceModel> notification_service) : _id(id), _type(type), _created_at(created_at), _updated_at(updated_at), _notification_service(notification_service) {}
...
std::vector<NotificationModel> NotificationModel::get_notifications_self(pqxx::connection& db, const std::string& user_id, bool throw_when_null) {
	...
	std::string query_services = R"(
        WITH user_related_services AS (
            SELECT
                ...
                n.updated_at AS notification_updated_at,
                ns.id AS notification_service_id,
                ns.sender_id AS service_sender_id,
                ns.service_id AS service_service_id,
                ns.status AS service_status
            FROM
                notifications n
            LEFT JOIN
	...
	// Placeholder for achievements subquery
	std::string query_achievements = R"(
        ...
        WITH user_related_achievements AS (
            SELECT
                ...
                n.created_at AS notification_created_at,
                n.updated_at AS notification_updated_at,
                NULL AS notification_service_id,
                NULL AS service_sender_id,
                NULL AS service_service_id,
                NULL AS service_status,
                na.id AS notification_achievement_id,
                ua.user_id AS achievement_user_id
            FROM
                notifications n
            LEFT JOIN
	...
	// Placeholder for reviews subquery
	std::string query_reviews = R"(
        /*
        WITH user_related_reviews AS (
            SELECT
                n.id AS notification_id,
                n.type AS notification_type,
                n.created_at AS notification_created_at,
                ...
                NULL AS service_service_id,
                NULL AS service_status,
                NULL AS notification_achievement_id,
                NULL AS achievement_user_id,
                nr.id AS notification_review_id,
                nr.review_id AS review_review_id
            FROM
                notifications n
            LEFT JOIN
                notifications_reviews nr ON n.id = nr.notification_id
            WHERE
                nr.user_id = $1
        )
        */
	...
	std::string final_query = query_services + " /* UNION ALL " + query_achievements + " UNION ALL " + query_reviews + " */ ORDER BY notification_updated_at DESC;";
