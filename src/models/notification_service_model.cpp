...
bool NotificationServiceModel::is_requested(pqxx::connection& db, const std::string& sender_id, const std::string& service_id) {
	try {
		pqxx::work txn(db);
...
bool NotificationServiceModel::refused_notifications(pqxx::connection& db, const std::string& service_id, const std::string& notification_id) {
	try {
		pqxx::work txn(db);
...
std::vector<std::unique_ptr<NotificationServiceModel>> NotificationServiceModel::get_notifications_accepted_self(pqxx::connection& db, const std::string& sender_id) {
	try {
		std::vector<std::unique_ptr<NotificationServiceModel>> notifications;
		pqxx::work txn(db);
		...
		for (const auto& row : result) {
			notifications.push_back(std::make_unique<NotificationServiceModel>(
