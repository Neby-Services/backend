...
void initialize_user_routes(NebyApp& app, pqxx::connection& db) {
	...
	CROW_ROUTE(app, "/api/users/<string>").methods(crow::HTTPMethod::PUT)([&db](const crow::request& req, crow::response& res, const std::string& user_id) {
		UserController::update_user_by_id(db, user_id, res);
	}
}
